...
var worker_default = {
  async fetch(request, env2) {
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: "Cuerpo inv\xE1lido" }), {
        status: 400,
        headers: { ...cors, "Content-Type": "application/json" }
      });
    }
    ...
    const values = [
      ...
      data.vehiculo_propio === true || data.vehiculo_propio === "true" || data.vehiculo_propio === "S\xED",
    ...
    try {
      const dbUrl = new URL(env2.DATABASE_URL);
      ...
      const dbRes = await fetch(neonHttpUrl, {
        ...
        headers: {
          ...
          "Neon-Connection-String": env2.DATABASE_URL
        },
